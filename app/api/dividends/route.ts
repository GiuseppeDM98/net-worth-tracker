...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dividendData.dividendType === 'coupon' && dividendData.isAutoGenerated) {
      ...
      // Asset sold (quantity 0): cleanup done, skip creating a zero-amount coupon entry
      if ((dividendData.quantity ?? 1) === 0) {
        return NextResponse.json({ success: true, message: 'Upcoming coupons cleaned up; no new entry created (asset sold)' });
      }
    }
    if (dividendData.dividendType === 'finalPremium' && dividendData.isAutoGenerated) {
      ...
      // Asset sold (quantity 0): cleanup done, skip creating a zero-amount final premium entry
      if ((dividendData.quantity ?? 1) === 0) {
        return NextResponse.json({ success: true, message: 'Final premium cleaned up; no new entry created (asset sold)' });
      }
    }

    // Create dividend entry — enrich with costPerShare snapshot for accurate historical YOC
