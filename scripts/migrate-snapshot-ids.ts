 ...
 *   npx tsx scripts/migrate-snapshot-ids.ts
 *
 *   # Execute migration
 *   npx tsx scripts/migrate-snapshot-ids.ts --execute
 *
 * Prerequisites:
 *   - Backup production database before running with --execute
 *   - Verify Firebase Admin SDK credentials are configured in .env.local
 */
...
if (result.error) {
  ...
  console.warn(`   Path attempted: ${envPath}`);
  console.warn('   Ensure .env.local exists in project root with Firebase credentials');
} else {
...
if (!process.env.FIREBASE_ADMIN_PROJECT_ID &&
  ...
  console.error('   - FIREBASE_SERVICE_ACCOUNT_KEY (JSON string), OR');
