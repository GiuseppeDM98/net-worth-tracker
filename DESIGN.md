...
- Motion is physics-native: spring dialogs, ease-out-quart state transitions, circle-reveal theme toggle — and, since 2026-09-12, three authored moments that reveal a relationship rather than decorate: a navigation is one scene, a period transforms instead of being replaced, and Storico's series is a month axis under the pointer (The Page Scene, The Period-Transforms Rule, The Scrub Rule)
...
- `lib/utils/cashflowSankey.ts` — the Sankey's node palette (twenty-one Tailwind-500 hexes): its
  ...
  slots, which would re-colour the meaning per theme.
...
- The Recharts `?? '#…'` fallbacks of Analisi's Dettaglio (`SavingsRateTrendSection.tsx`,
  ...
  slots have resolved.
- `canvas-confetti` in `RaddoppiTile.tsx` and `FireCalculatorTab.tsx`: a canvas cannot read a CSS
  variable, said in-file.
...
**The Scrub Rule (2026-09-12).** On a page with no period axis, ONE plotted series may become the axis for as long as a fine pointer is on it: the month under the pointer is lifted to the page, resolved once (`lib/utils/storicoScrub.ts`), and every tile that can honour it shows that month — the head value glides onto the figure (a 220 ms settle, never a count-up from zero), the ranked lists re-rank on the one spring, a marker names the month on the other plots, the slot lights on the bars — while every tile that cannot (a snapshot without a per-instrument breakdown, a month before the cashflow floor) stays where it was instead of inventing a state. Three constraints keep it a reading and not a mode. **The words do not move**: reading lines and verdict are the page's answer over the whole history, so they stay; only figures change, and the line that names the scrubbed month is reserved at a fixed height so the plot under the pointer never reflows. **Nothing is committed**: leaving the plot or pressing Escape returns to today, and a choice the user made (a Select) survives untouched. **The tooltip does not repeat the head**: while the head follows, the series' own card carries only what the head cannot (a note), and nothing without one. A coarse pointer never scrubs — the Hover Reading Rule holds, the chart is a shape there and its tooltip keeps the full reading. First on Storico (Evoluzione → Composizione, Valore per strumento, Driver).
...
- **Do** let a plotted series become the axis of an axis-less page under a fine pointer (The Scrub Rule): resolve the month once, move figures and not words, reserve the line that names the month, commit nothing, and let the series' own tooltip carry only what the head cannot.
