...
its footer stacks `flex-col-reverse` at `h-11`.
...
reading · figures). The ramp is DESIGN.md's own, divided by 4/3 — `PDF_RAMP` in
`components/pdf/primitives/PDFTile.tsx` — so the printed page carries the screen's hierarchy at the
same physical size; A4 is 595×842pt on a 44pt margin, leaving a 507pt column.
...
- **Do** use `useAnimation + useEffect([])` (empty deps) for "animate once on mount" ring charts. This prevents the ring from restarting whenever a parent component re-renders due to unrelated state changes. (No surface draws one since 2026-08-31 — the guidance is here for the next ring, not for a live one.)
