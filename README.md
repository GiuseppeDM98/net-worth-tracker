...
- **AI Assistant** with persistent conversation history, automatic memory, and unified data access: select any past month for a structured breakdown of net worth changes, cashflow, top expenses, and allocation shifts; free-form chat mode now receives the same real portfolio data as monthly analysis; the assistant learns and remembers your goals, preferences, risk profile, and stable facts across conversations — visible and editable in a dedicated Memory panel; suggested prompt chips, streaming markdown responses, per-thread month pinning, navigable conversation list, and mobile drawer for thread access
