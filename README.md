...
- **AI Assistant** with persistent conversation history, automatic memory, and unified data access: select any past month for a structured breakdown of net worth changes, cashflow, top expenses, and allocation shifts; free-form chat mode receives the same real portfolio data as monthly analysis; the assistant learns and remembers your goals, preferences, risk profile, and stable facts across conversations — visible and editable in a dedicated Memory panel; the numeric context panel (net worth delta, cashflow, allocation changes) reappears automatically when reopening a past monthly thread without rerunning the analysis; suggested prompt chips, streaming markdown responses (including tables), per-thread month pinning, sticky right panel with context and preferences, navigable conversation list on desktop and mobile drawer; web search for macro/geopolitical context with specific event citation; controlled rollout via feature flag
