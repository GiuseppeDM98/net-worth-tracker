...
- **Read any month under the mouse** (2026-09-12): move the pointer along the Evoluzione line and the whole page reads that month — the value at the top of the tile glides to that month's figure with its change on the month before, Composizione lists that month's split with its rows re-ranking as you move, Valore per strumento jumps to it when the snapshot has the per-instrument breakdown, and the Driver bars light that month. Leave the line or press Escape to come back to today; the words never change and nothing is saved. On a phone the line stays a shape and its tooltip keeps the reading
