...
- Ultima implementazione: **Performance Page Animations + Skeleton Loading** — `useCountUp` hook (ease-out-quart, 700ms, `requestAnimationFrame`) anima i valori numerici delle MetricCard al caricamento e al cambio periodo; stagger cascade entrata card (`slide-in-from-bottom-4`, 80ms tra card, 120ms tra sezioni); titoli sezione `slide-in-from-left-4`; `key={selectedPeriod}` forza remount e replay animazioni; skeleton screen sostituisce lo spinner di caricamento (riproduce layout reale: header, tab bar, 3 sezioni, grafici); hover lift su card; AI button glow viola. Fix: `Math.round()` su float in `formatValue` per `months` format; `h-full` su wrapper + Card per equal-height in grid. (2026-03-15)
