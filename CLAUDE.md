...
- Ultima implementazione: **Dashboard KPI count-up animation** — `useCountUp` estratto da `MetricCard` in `lib/utils/useCountUp.ts` con opzione `once: true`; applicato ai 6 KPI principali della dashboard (Patrimonio Totale/Liquido Lordo, Totale/Liquido Netto, Plusvalenze, Tasse Stimate). Animazione parte solo al mount, non si ritrigger su refresh dati. (2026-03-29)
- Precedente: **Design Context setup** — creato `.impeccable.md` con personalità brand (Elegante·Sofisticato·Personale), riferimenti (Linear/Vercel), direzione (Overdrive) e 5 principi guida per le skill impeccable. (2026-03-29)
