...
- Latest implementation (2026-04-29, session unified-whale): **Email category breakdown + AI comment rendering fixes**. `monthlyEmailService.ts`: `aggregateExpenses()` now tracks income categories (`allIncomeCategories`) and returns all expense categories without cap. `MonthlyEmailData` gains `allIncomeCategories`. Email HTML: new "Entrate per Categoria" table (all income categories, EUR + %) after cashflow summary; "Spese per Categoria" (renamed from "Top Categorie di Spesa") shows all categories with % column. `simpleMarkdownToHtml()` rewrite: collapsed `</li>\n\n<li>` before `<ul>` wrapping; `\n\n` → `<br/><br/>` with post-processing to strip `<br/>` around heading `<p>` and reduce to single `<br/>` around `<ul>/<ol>`; added `<ol>` support via placeholder pattern with more spacing per item; added `*italic*` → `<em>`; strip `<details>/<summary>` that Claude occasionally emits; increased heading `margin-top` from `8px` to `16px`. Tests: 81 pass.
- Previous (2026-04-28, session jazzy-fountain): **Contesto macro in chat libera**. Il toggle `includeMacroContext` ora funziona anche in chat libera (non solo nelle analisi strutturate). `resolveAssistantWebSearchPolicy` in `webSearchPolicy.ts`: in chat mode, `return preferences.includeMacroContext || shouldUseWebSearch(prompt)` — toggle ON = web search sempre attivo, toggle OFF = fallback a keyword detection. Microcopy aggiornato: toggle description e guida "Come funziona" in `AssistantPageClient.tsx`. Test aggiornati: 3 casi per chat mode in `assistantWebSearchPolicy.test.ts` (7 test pass). Tests: 93 pass.
- Previous (2026-04-26, session ai-email-comment): **AI comment in periodic emails**. All three email types (monthly/quarterly/yearly) now include a "Commento AI" section generated by the same assistant analysis pipeline used in the interactive UI. New `quarter_analysis` mode added to `AssistantMode` — adds `buildAssistantQuarterContext()` in `assistantMonthContextService.ts`, `buildQuarterAnalysisPrompt()` in `prompts.ts`, wired into `anthropicStream.ts`, `webSearchPolicy.ts`, and `store.ts`. Email service (`monthlyEmailService.ts`): added `aiComment?: string` to `MonthlyEmailData`, `simpleMarkdownToHtml()` (regex-based, no new deps), `generateEmailAiComment()` (resilient — AI failure never blocks email). `buildAndSendForPeriod` now generates AI comment before sending. Cron route monthly Phase 2 now uses `buildAndSendForPeriod` (consistent with quarterly/yearly). `AssistantMonthContextBundle.selector` gains `quarter?: number` so quarterly renders "Q1 2026" not "Marzo 2026". Tests: 93 pass.
- Previous (2026-04-19, session periodic-email-enrichment): **Periodic email summaries (monthly/quarterly/yearly) with richer content**. Three independent toggles in Settings → Preferenze (`monthlyEmailEnabled`, `quarterlyEmailEnabled`, `yearlyEmailEnabled`), shared recipient list (`monthlyEmailRecipients`). Cron `monthly-snapshot` now runs 4 phases: snapshots, monthly email (last day of month), quarterly email (last day of March/June/September/December), yearly email (December 31). Manual send via `POST /api/user/monthly-email/send` with body `{ periodType: 'monthly'|'quarterly'|'yearly' }` — auto-resolves most recent completed period (April 19 → Q1 for quarterly, 2025 for yearly). Email content: % allocation column, "Performance Asset Class" with separate Δ% and Δ€ sub-tables (best/worst each), top 3 expense categories, top 5 individual expense transactions (uses `notes` field as description), savings rate %. Architecture: unified `buildPeriodEmailData(userId, year, month, periodType)`. Tests: 78. Env vars: `RESEND_API_KEY`, `RESEND_FROM_EMAIL`.
...
**Last updated**: 2026-04-26 (session ai-email-comment — AI narrative comment in all periodic emails, quarter_analysis mode)
