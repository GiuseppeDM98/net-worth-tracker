...
- Ultima implementazione: **Mobile responsive audit — Dividends + Budget** — `DividendStats` pie chart top 7 + "Altri", DPS Growth card+dialog mobile, Total Return card view mobile; `DividendTrackingTab` filtri full-width; `BudgetTab` `MobileAnnualView` card per item con detail dialog, `CategoryDeepDive` (Analisi Storica) desktop-only (`hidden desktop:block`), item card completamente tappabile. (2026-03-18)
...
**Last updated**: 2026-03-18 (session: Dividends + Budget mobile responsive audit)
