...
- Latest (2026-06-20): **Allocazione redesign (impeccable, "ripensamento")** — hero now pairs the dominant total with `AllocationCompositionBar` (stacked composition by class; theme colors via `useChartColors` aligned to History's `acColors` through `ASSET_CLASS_CHART_INDEX`) and `BalanceScoreGauge` (radial ring; band-independent `computeBalanceScore` = 100 − ½·Σ|drift|; color band via `useActionColors`). `RebalancePlan` + `ContributionAllocator` merged into `ActionPlanner` (segmented Ribilancia/Versa → `RebalancePanel`/`ContributionPanel`); `RebalanceBandControl` reframed as a labeled strip; page split into a Decision zone → "Dettaglio" divider → Detail zone. Read-only, no demo gating.
...
**Last updated**: 2026-06-20 (Allocazione redesign: `AllocationCompositionBar` + `BalanceScoreGauge` in the hero, `ActionPlanner` segmented Ribilancia/Versa, band-independent `computeBalanceScore`, Decision/Dettaglio zones. See Current Status → Latest.)
