...
- Ultima implementazione: **Hall of Fame Mobile/Tablet Audit** — audit `/adapt` pagina Hall of Fame: `lg:grid-cols-2` → `desktop:grid-cols-2` nei grid mensile/annuale; `md:hidden`/`hidden md:block` → `desktop:hidden`/`hidden desktop:block` per i 16 switch card/tabella; `max-desktop:portrait:pb-20` sul root wrapper; dialog note `grid-cols-2` → `grid-cols-1 sm:grid-cols-2`; touch target NoteIconCell `w-8 h-8` → `w-10 h-10`. (2026-03-18)
