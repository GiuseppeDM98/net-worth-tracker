...
- Latest implementation (2026-04-03, session 40): **Page transition flash fix**. Replaced `AnimatePresence mode="wait"` + `motion.div key={pathname}` in `layout.tsx` with a new `app/dashboard/template.tsx`. `template.tsx` re-mounts on every navigation (unlike `layout.tsx` which persists), guaranteeing Framer Motion applies `initial="hidden"` correctly without inheriting the previous variant context. Removed redundant `motion.div pageVariants` outer wrapper from `app/dashboard/page.tsx` (Panoramica); other pages still have the wrapper and should be cleaned up in future sessions. Trade-off: no exit animation on page leave.
