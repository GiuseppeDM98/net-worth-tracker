...
- Latest implementation (2026-05-20, session dividend-stats-yoc): **YOC card enhancement + timezone bug fix**. (1) Exposed `totalTtmDividends` in `yocSummary` memo return (`components/dividends/DividendStats.tsx`); added "Dividendi/Cedole TTM (Lordo)" third column to the YOC card detail grid (`grid-cols-1 sm:grid-cols-3`). (2) Fixed `paymentDate <= today` filter in `dividendService.ts`, `app/api/dividends/stats/route.ts`, `lib/server/dividendUseCase.ts`: changed `today.setHours(0,0,0,0)` → `today.setHours(23,59,59,999)` so dividends with `paymentDate` at midnight UTC are never misclassified as future on CEST servers (UTC+2 produces `22:00Z` at local midnight, 2h before the stored timestamp).
- Previous (2026-05-17, session issue-128-stock-sectors): **Esposizione Portfolio — stock sector attribution fix**. Single stocks were missing from the Sectors tab. Root cause: sector aggregation iterated only over `etfData` (Yahoo `topHoldings.sectorWeightings`). Fix: added parallel `assetProfile` fetch for stocks; `YAHOO_ASSET_PROFILE_SECTOR_TO_KEY` map converts title-case strings; new sector loop adds each stock at 100% weight. ETF+stock fetch concurrent via `Promise.all`. `cacheKey` extended to include stock tickers. Key file: `lib/server/portfolioExposureService.ts`.
- Previous (2026-05-17, session refactor-usewatch): **React Compiler compatibility — `watch()` → `useWatch()` migration** across all 4 form dialogs. `AssetDialog` (34 calls), `ExpenseDialog` (11 calls, `"use no memo"` directive removed), `CategoryManagementDialog` (5 calls), `DividendDialog` (4 calls). Rule: `useWatch({ control, name })` for render-time reads; `getValues()` for event handlers.
- Previous (2026-05-16): **ExpenseDialog + AssetDialog 2-step redesign**. Step 1 = visual type picker; Step 2 = form with fields filtered by type. Edit mode lands directly on Step 2. Key files: `components/expenses/ExpenseDialog.tsx`, `components/assets/AssetDialog.tsx`.
- Previous (2026-05-13, session portfolio-exposure-breakdown): **Esposizione Portfolio v1**. Collapsible section at bottom of Allocazione with 3 tabs (top companies, sectors, ETF issuers). `GET /api/portfolio/exposure`, 24h Firestore cache, `?force=true` bypass. New files: `types/exposure.ts`, `lib/server/portfolioExposureService.ts`, `app/api/portfolio/exposure/route.ts`, `lib/hooks/usePortfolioExposure.ts`, `components/allocation/ExposureSection.tsx`.
...
**Last updated**: 2026-05-17 (session issue-128-stock-sectors — stock sector attribution fix in Esposizione Portfolio)
