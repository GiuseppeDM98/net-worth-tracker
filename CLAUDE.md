...
- Latest implementation (2026-04-13, session coast-fire-pension-statale-polish): **Final polish pass on the Coast FIRE state pension UI** (`components/fire-simulations/CoastFireTab.tsx`). Six targeted fixes: (1) toast validation errors corrected to "età" (was "eta"); (2) timeline detail string `"E' gia' attiva"` → `"È già attiva"`; (3) Proiezione chart card now has `border-border/70` consistent with all other cards; (4) nested "Assunzioni del modello" collapsible chevron now rotates on open via `group` + `group-data-[state=open]:rotate-180`; (5) "Vista compatta" badge removed — was internal layout info, not user-meaningful; (6) unsaved-changes banner now shows `Info` icon at rest and `Loader2 animate-spin` only while saving. No logic or data-flow changes.
