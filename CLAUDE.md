...
- Latest implementation (2026-04-09, session arrange): **Layout & spacing rhythm, Assistente AI**. Message spacing `space-y-3→4`, bubble padding `py-3→4`, header gap `mb-2→2.5`. Composer: `pt/pb-3→4`, mode-row `mb-2→3`, added upward shadow (`rgba` hardcoded — Tailwind v4 has no semantic upward-shadow token; dark variant `0.3` vs `0.06` light because dark surfaces absorb contrast). ContextCard: hero KPI `p-3→4`, section labels `mb-1→2`, `space-y-4→5`. MemoryPanel: category groups `space-y-4→5`, items `space-y-1.5→2`. PageClient: hero `mb-4→6`, messages pane `p-4→5`, right column `space-y-4→6`. Previous session (a11y-perf): `aria-live` streaming, `MARKDOWN_COMPONENTS` module-level, `scrollIntoView instant` during stream, touch targets `h-8 w-8`, memory filter `role="tablist/tab"`, `aria-label` all controls.
