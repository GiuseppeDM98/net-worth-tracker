...
- Latest implementation (2026-04-09, session adapt-mobile): **Assistente AI mobile UX**. Composer: mode Select replaced by horizontal scrollable chip strip on mobile (`desktop:hidden`); chat context type also chips; removed 88px dead padding (`max-desktop:portrait:pb-[88px]` was redundant — sticky `bottom-20` wrapper already handles BottomNav clearance); added `pb-[env(safe-area-inset-bottom,0px)]` for iPhone home bar; keyboard hint hidden on mobile. Memory panel: accessible on mobile via Brain icon Sheet in header; hidden from right-column scroll flow (`hidden desktop:block`). Context card: `AssistantContextPill` (new export) shows delta + pct inline in conversation header on mobile; full card hidden from right column on mobile. Header buttons: `flex-col` on mobile (two full-width rows) vs `flex-row` on desktop; each row uses `flex-1` on text buttons. Quick-chip strip in conversation header removed (superfluous). `CollapsibleTrigger` in `AssistantMemoryPanel` is always `asChild` — avoids nested `<button>` hydration error when `collapsible=false`. Previous session (arrange): layout & spacing rhythm — message spacing, bubble padding, upward shadow, right column rhythm.
...
- Assistente AI (2026-04-09): 5 modes — `month_analysis`, `year_analysis`, `ytd_analysis`, `history_analysis`, `chat`. Chat mode has `chatContextType` selector (`none | month | year | ytd | history`), default `'none'`. `bySubCategoryAllocation` in context bundle. `selector.month` encoding: `>0`=monthly, `0`=year, `-1`=YTD, `-2`=history. `useAssistantPeriodContext` unifies all 4 context hooks. `includeMacroContext` gates web search. Extended thinking (budget 2000) + 3500 max_tokens. `resolvedThreadId` pattern avoids stale RQ cache. Feature flag `NEXT_PUBLIC_ASSISTANT_AI_ENABLED`. Persistent conversations, auto-memory, markdown with tables. Memory extraction runs in all modes, gated by `memoryEnabled` only. `includeDummySnapshots` preference: toggle visible only when `hasDummySnapshots`. **Stop**: `AbortController` in `abortControllerRef`. **Delete confirmations**: 2-click inline with 3s auto-disarm. **Mobile (2026-04-09)**: `AssistantComposer` uses horizontal chip strip for mode + chat context on mobile (`desktop:hidden`); desktop keeps Select dropdowns. Removed 88px dead pb (sticky wrapper handles BottomNav clearance). `pb-[env(safe-area-inset-bottom,0px)]` for iPhone home bar. Memory panel hidden from right-column on mobile — Brain Sheet in header opens it. `AssistantContextPill` (exported from `AssistantContextCard`) shows delta+pct inline in conversation header on mobile. Right-column context card + memory panel wrapped in `hidden desktop:block`. Header buttons: `flex-col` two-row on mobile, `flex-row` on desktop. `CollapsibleTrigger` always `asChild` in `AssistantMemoryPanel` (prevents nested `<button>` when `collapsible=false`). **Layout**: Preferences in header popover (Settings2). Memory collapsible in sidebar. **Conversation history**: cap chat 10 pairs, structured 3 pairs. **A11y**: `aria-live` on stream container; `role="tablist/tab"` on memory filters; action buttons always visible on touch; `scrollIntoView instant` during streaming.
...
**Last updated**: 2026-04-09 (session adapt-mobile — composer chips, memory sheet, context pill, iOS safe area)
