...
- Ultima implementazione: **Framer Motion su Performance e Dividendi** — `staggerContainer` + `cardItem` (variant propagation, no compound-opacity) sui 6 chart card di `performance/page.tsx`; `pageVariants` + `cardItem` con explicit delays su `DividendTrackingTab.tsx`. (2026-03-22)
- Precedente: **Button Micro-interactions** — `hover:-translate-y-[1px]` + `active:scale-[0.97] active:translate-y-[1px]` + `duration-150` nella base class di `components/ui/button.tsx` (cva). Override `hover:translate-y-0` su `ghost`, `link`, `icon`, `icon-sm`, `icon-lg`. (2026-03-22)
...
**Last updated**: 2026-03-22 (session 13: Framer Motion — performance & dividends)
