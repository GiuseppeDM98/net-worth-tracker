...
- Ultima implementazione: **Labor Income KPI cards + All-months savings chart** — (2026-03-30, session 21). Dashboard: 4 nuove card condizionali (Guadagnato/Risparmiato da Lavoro + Crescita Investimenti Lordo/Netto) attive solo se `laborIncomeCategoryIds` configurato in Settings; fetch lazy via `useAllExpenses` hook. Settings Preferenze: multi-select pill per categorie income → `laborIncomeCategoryIds` in `AssetAllocationSettings`. History: grafico "Risparmio vs Crescita" vista mensile ora ha opzione "Tutti" per timeline continua multi-anno via `prepareSavingsVsInvestmentDataAllMonths`.
- Precedente: **EmptyState component** — `components/ui/EmptyState.tsx`; 5 SVG icon exports; sostituiti 7 empty state. **Animated Toast Icons** — `AnimatedCheckIcon/ErrorIcon/WarningIcon`; SVG draw via `stroke-dashoffset`. (2026-03-30)
...
**Last updated**: 2026-03-30 (session 21: labor income KPI cards + all-months savings chart)
