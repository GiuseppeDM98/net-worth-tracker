...
- Latest implementation (2026-04-10, session themes-fix): **Dark theme chroma fix + 2 new themes**. Fixed near-zero chroma in `.dark[data-theme="..."]` CSS var blocks (solar-dusk, elegant-luxury, midnight-bloom) — tables/cards now show a visible tint in dark mode. elegant-luxury dark had wrong hue (copied from solar-dusk). Added `cyberpunk` (neon pink + electric teal) and `retro-arcade` (red + teal on warm yellow-green bg) from tweakcn. Now 6 selectable themes total. Removed hardcoded `dark:bg-gray-700` from 6 buttons in Settings. Previous session (2026-04-09, themes): multi-theme system, view transition, chart colors, bottom nav sync.
