...
- Latest implementation (2026-04-09, session harden): **Stop button, delete confirmations, thread item layout fix**. `AbortController` wired to SSE fetch in `handleStreamSubmit`; stop button (Square icon, `variant="destructive"`) replaces send during streaming, always enabled. Abort detected via `error.name !== 'AbortError'` — skips toast, leaves partial text visible with "Risposta interrotta" state. Memory item delete: 2-click inline confirmation (`isPendingDelete` + 3s auto-disarm timer). Conversation (thread) delete: same pattern with `pendingDeleteId: string | undefined` inside `ThreadList`. Thread card layout: delete button moved from `absolute right-2 top-2.5` to `flex shrink-0` sibling — was overlapping the mode Badge. Previous sessions: clarify (mode descriptions), distill (toolbar simplification), 68c (sidebar restructure).
