...
- Latest implementation (2026-04-13, session coast-fire-pension-responsive): **Responsive adaptation of the Coast FIRE state pension UI**. The feature was previously a vertical stack of the desktop layout on mobile. Changes in `components/fire-simulations/CoastFireTab.tsx`: 3-card summary header drops `sm:grid-cols-2` (no orphan card on tablet); pension editor inputs move to `grid-cols-2 items-end` on mobile (2×2 pairing, baseline-aligned even when labels wrap); Bear/Base/Bull cards gain `sm:grid-cols-2`; pension breakdown table restructured from 5 stacked blocks to a compact header + 2-col metric grid; tax bracket rows now render inline on all viewports (`grid-cols-[1fr_100px_44px]`); trash buttons bumped to `h-10 w-10` (40px touch target); "Aggiungi pensione" becomes `w-full` on mobile. Two user-visible em-dashes replaced with commas in pension issue messages.
