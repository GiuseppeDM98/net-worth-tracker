...
- Latest implementation (2026-04-13, session coast-fire-pension-statale-audit): **Audit + UX/accessibility fixes on Coast FIRE state pension UI** (`components/fire-simulations/CoastFireTab.tsx`). Key changes: (1) hint text "lordo nominale futuro" added below importo field — critical for correct user input; (2) "Situazione a regime" CardDescription now conditional on `pensionBreakdown.length > 0`; (3) gap card shows "Target raggiunto" + "✓ Coast FIRE" when `isCoastReached`; (4) config panel no longer auto-collapses after save (one-way auto-open only); (5) `role="progressbar"` + ARIA attrs on progress bar; (6) `aria-live="polite"` on unsaved-changes banner; (7) duplicate progress bar removed from Coast Number card; (8) timeline "A regime" step conditional on `bridgeYears > 0`; (9) `items-start` replaces `items-end` on pension input grid (hint text broke bottom-alignment); (10) trash icon always `flex-row` in pension card header; (11) Scaglioni IRPEF section header now stacks on mobile like pension section; (12) `buildPensionDraftIssues` receives `now` as explicit parameter; (13) semantic colors `emerald/sky/amber -200` → `-300`.
