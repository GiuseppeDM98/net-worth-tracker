...
- Ultima implementazione: **FIRE & Simulazioni Mobile/Tablet Audit** — audit `/adapt` pagina FIRE: Tab → Radix Select su `max-desktop:`; `max-desktop:portrait:pb-20` sul root; header `text-2xl sm:text-3xl`; tutti i `md:grid-cols-X` → `desktop:grid-cols-X` in FireCalculatorTab/FIREProjectionSection; chart height responsivi via `useMediaQuery` (280px mobile, 400px desktop); FIREProjectionChart accetta prop `height`/`marginLeft`; card view mobile in FIREProjectionTable (`desktop:hidden`) e MonteCarloTab percentili; GoalBasedInvestingTab header flex-col, bottone `w-full sm:w-auto`; touch target GoalDetailCard remove button `h-10 w-10`. (2026-03-18)
