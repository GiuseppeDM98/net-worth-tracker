...
- Ultima implementazione: **Budget Tab — Analisi Storica Aggregata** — subtotal e footer row (Totale Spese/Entrate) ora cliccabili nel tab Budget (vista annuale); chiavi sentinel `__subtotal_{type}__`/`__total_expenses__`/`__total_income__`; `deepDiveData` useMemo esteso con branch aggregato; "Totale Spese" mostra 3 mini-tabelle Anno×Mese per tipo (Fisse/Variabili/Debiti) con stessa logica highlight. (2026-03-18)
...
- **Budget Tab (Cashflow)**: auto-init items da tutte le categorie spesa/entrata; vista annuale con Budget/anno vs anno corrente/precedente/media storica e progress bar; deep dive storico: click su qualsiasi riga (categoria, subtotale sezione, Totale Spese/Entrate) → pannello inline “Analisi Storica” Anno×Gen…Dic (tutti gli anni da historyStartYear), highlight min/max mese per riga-anno (rosso/verde, invertito per Entrate). **“Totale Spese”** mostra anche 3 mini-tabelle Anno×Mese per tipo (Spese Fisse / Variabili / Debiti) con stessa logica highlight. Chiavi aggregate: `__subtotal_{type}__`, `__total_expenses__`, `__total_income__`. Sezioni collassabili; riordino items; add subcategory inline; sezione Entrate con colori invertiti; guida contestuale. Footer mostra **Totale Spese** e **Totale Entrate** separati. Firestore: `budgets/{userId}` doc singolo. 18 unit test.
