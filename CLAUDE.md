...
- Ultima implementazione: **Page Transitions & Skeleton Screens** — `AnimatePresence mode="wait"` in `app/dashboard/layout.tsx` con `key={pathname}`; variante `exit` aggiunta a `pageVariants` (opacity 0, y -8px, 150ms ease-in); `HistoryPageSkeleton` per la pagina Storico (8 chart card + snapshot grid, stagger 100-800ms); skeleton per tab FIRE (`FireCalculatorSkeleton`, `MonteCarloSkeleton`, `GoalsSkeleton`); allocation/hall-of-fame/settings usano `return null` durante loading (troppo veloci per skeleton). Fix aria warning su `SheetContent` (`aria-describedby={undefined}`). `useDelayedLoading` hook in `lib/hooks/`. (2026-03-21)
- Precedente: **Tooltip Colors Fix** — History page: rimosso `color` da `itemStyle` in 8 tooltip + `fill` alla `<Bar>` YoY. Overview: custom tooltip in `components/ui/pie-chart.tsx` che legge `entry.payload.color`. (2026-03-20)
...
- History page: Net Worth evolution, Asset Class breakdown, Liquidity, YoY variation, Savings vs Investment Growth (toggle Annuale/Mensile con selettore anno nella vista mensile), Doubling Time Analysis (geometrico + soglie fisse, summary cards adattivi alla modalità), Current vs Target allocation. **Animazioni Framer Motion**: page fade-in, chart card cascade con delay esplicito, `AnimatePresence` + `slideDown` per Note, stagger snapshot grid, milestone card stagger + progress bar animata. **Skeleton screen**: `HistoryPageSkeleton` (header + 8 chart card + snapshot grid) al posto del testo "Caricamento...".
...
- Animation: `lib/utils/motionVariants.ts`, `components/providers/MotionProvider.tsx`
...
**Last updated**: 2026-03-21 (session 10: page transitions + skeleton screens)
