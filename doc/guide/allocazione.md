...
- **A drift wears no sign token.** Every figure of `allocazioneNarrative.ts` is `mono` and uncoloured; the action hues (`useActionColors`, resolved ONCE per tile and passed down — `MoveRow` takes the map, `PlanRow` the one colour of its move) colour the chips, the plan amounts and the ring only.
...
- **`PlanView` is a discriminated union and the Piano narrows on `view.mode`, never on the toggle**; it draws `moves`/`nodes` with or without leverage, and `trades` (the leverage engine's own orders) feed only the reading and the verdict — see *The leverage engine's trades are the same tree* below. `MIN_VISIBLE_AMOUNT` lives in `allocazioneSummary.ts` (re-exported by `PlanRow`). A Ribilancia with nothing to do draws no body: the reading already says «Tutto in linea», and «a saldo zero» is said only when Σsell and Σbuy agree within a euro (a class inside the band keeps its gap).
...
- **«Prelevare 1000 €» vuol dire 1000 € IN MANO** (`solveWithdrawalGross`, 2026-09-21): il piano vende il LORDO che, tolta la ritenuta, lascia la cifra chiesta. **Non è una divisione per (1 − aliquota)**: la ritenuta dipende da QUALI strumenti il piano drena e da quanta plusvalenza porta ciascuno, e quali strumenti drena dipende dall'importo — è un punto fisso, `gross = netto + tax(plan(gross))`, risolto per iterazione (converge perché la tassa cresce molto più lentamente del lordo). Tre uscite oneste: ritenuta non stimabile → vende la cifra chiesta e non promette alcun netto (`grossedUp: false`); niente plusvalenza → lordo uguale al netto; lordo oltre il negoziabile → `exceedsPortfolio`, e la frase dice che il netto non si raggiunge. Le RIGHE sommano il lordo, quindi la frase lo nomina: chi le controlla deve ritrovare la cifra stampata. Il motore con leva si lorda con lo stesso punto fisso (`solveWithdrawalGross` prende il pianificatore come argomento): al motore si chiede il LORDO, e la ritenuta si stima sulle gambe che vende.
- **The leverage engine's trades are the same tree** (2026-09-24). `planInstrument*` trades whole ASSETS; `groupFlowTrades` / `groupRebalanceTrades` (`allocazioneSummary.ts`) turn its orders into the class → instrument tree the plan without leverage draws, so the Piano reads the same on both engines and `estimatePlanSaleTax` prices the sold legs (before, a leveraged plan had no tax, no net and a flat list). Three rules: **a composite order is split across its classes by composition** — the sleeves `buildHoldings` already emits, found through `AllocatableHolding.assetId`, each keyed by its holding id and carrying the whole order in `PlanNode.order` («parte di un ordine da 2000 € di NTSG» — the ticker in the caption, since the name truncates on a phone), because a 60/40 leveraged ETF IS in two classes and the tree is read by class; **a Ribilancia that sells and buys inside one class is TWO moves**, one per action (the engine swaps a 1× ETF for a 2× one to move the leverage), never a net no broker fills — `MoveRow`'s key is class AND action; **the class row's «→ %» is the engine's**, notional over the post-trade market base (`resultingNotionalByAssetClass`), never re-derived from the legs' market euros. A trade no holding matches (not expected: both come from one asset list) lands under «Altri strumenti» as an unpriceable leaf, never dropped.
...
- **Allocazione**: `e2e/allocation.spec.ts` since 2026-09-21 (roving focus, the row's own figures, the sub-category base, the rebalance's descent, no sideways scroll) — it does NOT cover the withholding, whose figure depends on cost bases the base fixture does not carry, nor the leverage engine, since the base fixture holds no leveraged asset (its tree is pinned by Vitest, `buildPlanView under leverage`, and seen on the mirror); Esposizione fetches `/api/portfolio/exposure` on mount (Yahoo on the first visit, then the 24 h cache) and truncates names at 128 px; a class held WITHOUT a target never enters `byAssetClass` (`compareAllocations` iterates the targets), so the score charges it as drift — the Bilanciamento reading names it, the verdict lists only targeted classes; a Ribilancia is «a saldo zero» only when the in-band classes carry no gap; «Modifica target» points to Impostazioni even with goal-derived targets; theoretical specific-asset targets are rows without a tick; `BandToggle` snaps 2 or 5 typed in the custom field back to the preset.
