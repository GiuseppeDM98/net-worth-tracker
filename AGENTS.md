...
- **Filter sections on mobile**: `flex flex-wrap gap-4` with `min-w-[Xpx]` items does NOT expand items to fill the row — each item takes only its minimum width, leaving visible gaps. Fix: `grid grid-cols-1 gap-3 desktop:flex desktop:flex-wrap desktop:items-end desktop:gap-4`. On mobile each filter becomes full-width; on desktop the original flex-wrap is restored via the `desktop:` prefix.
- **Quick-action button paired with a filter**: when a button is logically paired with a filter control (e.g. "Mese corrente" next to the month select), merge them in a single grid item as a flex row — `<div class="flex gap-2"><div class="flex-1 min-w-0">…Select…</div><Button class="shrink-0">…</Button></div>`. This avoids a separate spacer-label `&nbsp;` item that breaks the single-column grid on mobile.
- **Currency value font in 2-column card grids**: `text-2xl` (24px) overflows card content areas (~140px wide) on mobile for amounts ≥ 6 digits. Use `text-lg desktop:text-2xl` — 18px on mobile/tablet, 24px on desktop.
