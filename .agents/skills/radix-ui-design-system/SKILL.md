...
```tsx
// âŒ Don't fight pre-styled components
<Button className="override-everything" />

// âœ… Radix gives you behavior, you add styling
<Dialog.Root>
...
```tsx
// âœ… Render as Next.js Link but keep Radix behavior
<Dialog.Trigger asChild>
  <Link href="/settings">Open Settings</Link>
</Dialog.Trigger>

// âœ… Render as custom component
...
1. **Don't skip accessibility parts**
   ```tsx
   // âŒ Missing Title and Description
   <Dialog.Content>
     <div>Content</div>
   </Dialog.Content>
   ```
